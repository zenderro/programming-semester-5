...
Тогда для любого начального вектора $x^{(0)}$, не перпендикулярного собственному вектору $e_1$, следующий итерационный процесс сходится $λ_1$, а вектора $x^{(k)}$ сходятся к вектору, коллинеарному собственному вектору $e_1$:
...
т.ч. $Σ(A_k) < Σ(A_{k-1})$ и $\lim Σ(A_k) → 0$.

В методе Якоби $O_k = T_{ij}(φ_k)$, $φ_k$ подбирается так, чтобы обнулить $a_{ij}$.

Для данных $i, j$ положим $x = -2a_{ij}$, $y = a_ii - a_jj$, если $a_{ii} ≠ a_{jj}$ (в~противном~случае~положим~$φ = \frac{π}{4}$).
...
Вектор $x_i$ строится как для приведения матрицы к треугольному виду методом отражений с единственным отличием — он строится для части столбца матрицы *под* диагональю, *не включая* диагональный элемент. 
